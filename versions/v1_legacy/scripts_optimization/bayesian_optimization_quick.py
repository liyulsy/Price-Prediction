...
# --- Optimization Parameters (简化版本) ---
N_TRIALS = 10  # 减少试验次数，快速测试
...
def train_and_evaluate(trial, price_seq_len, train_loader, val_loader, edge_index, scaler, dataset):
    ...
    # MLP 参数
    mlp_hidden_dim_1 = trial.suggest_categorical('mlp_hidden_dim_1', [256, 512])
    mlp_hidden_dim_2 = trial.suggest_categorical('mlp_hidden_dim_2', [128, 256])
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=2, factor=0.7, min_lr=1e-6)
...
def objective(trial):
    ...
    # 首先定义需要影响数据集的超参数
    price_seq_len = trial.suggest_categorical('price_seq_len', [30, 60, 90, 120])
...
"""
    ...
    config_path = os.path.join(OPTIMIZATION_RESULTS_DIR, f'best_config_quick_{datetime.now().strftime("%Y%m%d_%H%M%S")}.py')

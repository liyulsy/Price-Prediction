...
class TimeXerConfigs:
    ...
    def __init__(self, num_nodes, price_seq_len, num_time_features, # 节点数，价格序列输入长度，时间特征数量
                 ...
                 patch_len=12, freq='h', output_attention=False, embed_type='timeF', # Patch长度，Patch滑动步长，时间序列频率，是否输出注意力权重，时间特征编码类型
...
D_MODEL = 64 # TimeXer模型主要特征维度
PATCH_LEN = 24 # TimeXer Patch长度
STRIDE = 12 # TimeXer Patch滑动步长
E_LAYERS = 2 # TimeXer编码器层数
N_HEADS = 4 # TimeXer Attention头数量
D_FF = 128 # TimeXer 前馈网络隐藏层维度
DROPOUT_TIMEXER = 0.1 # TimeXer Dropout比例
...
BEST_MODEL_PATH = "Project1/cache/best_timexer_gcn_model_v2.pt" # 最佳模型保存路径

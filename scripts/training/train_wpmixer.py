...
# 导入WPMixer模型：结合了小波分解、补丁混合和图神经网络的统一模型
        ...
        for batch_data in tqdm(data_loader, desc="Evaluating"):
            ...
            if task_type == 'classification':
                ...
                if PREDICTION_TARGET == 'price':
                    # 直接预测价格
                    targets = target_data
                    # 调整维度匹配：outputs是[batch, num_coins, 1]，targets是[batch, num_coins]
                    outputs = outputs.squeeze(-1)  # 移除最后一个维度：[batch, num_coins, 1] -> [batch, num_coins]
                else:
...
if __name__ == '__main__':
    ...
    class WPMixerConfigs:
        def __init__(self):
            ...
            self.pred_length = 1
            self.num_coins = dataset.num_coins
    ...
    # 5. Initialize WPMixer Model
    print(f"🚀 初始化wpmixer模型，任务类型: {TASK_TYPE}, GCN配置: {GCN_CONFIG}")
    ...
    print(f"📋 模型配置:")
    print(f"  WPMixer: input_len={PRICE_SEQ_LEN}, d_model={D_MODEL}, patch_len={PATCH_LEN}")
    print(f"  新闻特征: {'启用' if USE_NEWS_FEATURES else '禁用'}")
    print(f"  GCN: {'启用' if USE_GCN else '禁用'}")

...
# --- 关键修改：导入新的混合重构模型 ---
from models.MixModel.hybrid_reconstruction_wpmixer import HybridReconstructionWPMixer
...
BEST_MODEL_NAME = "best_wpmixer_model.pt"  # 最佳模型文件名

# --- Dataset Parameters ---
# 数据集参数配置
COIN_NAMES = ['BTC', 'ETH', 'BNB', 'XRP', 'LTC', 'DOGE', 'SOL', 'AVAX']  # 要分析的加密货币列表
PRICE_SEQ_LEN = 90               # 价格序列长度：保持96以匹配patch配置
...
# --- WPMixer Model Parameters ---
# WPMixer模型架构参数
D_MODEL = 256                   # 模型维度：恢复到128以提高模型容量
PATCH_LEN = 16                   # 补丁长度：保持8以匹配序列长度
PATCH_STRIDE = 8                # 补丁步长：保持4
# NUM_MIXER_LAYERS = 4          # 已移除：新架构中混合层数量由内部架构决定
WAVELET_NAME = 'db2'            # 小波类型：Daubechies 4小波
WAVELET_LEVEL = 1              # 小波分解层数
TFACTOR = 5                     # Token混合器扩展因子
DFACTOR = 5                     # 嵌入混合器扩展因子

NEWS_PROCESSED_DIM = 64         # 新闻特征处理后的维度：将原始新闻特征压缩到64维
GCN_HIDDEN_DIM = 128           # GCN隐藏层维度：减少复杂度，避免过拟合
GCN_OUTPUT_DIM = 64            # GCN输出维度：与输入维度匹配，保持信息流
MLP_HIDDEN_DIM_1 = 256        # MLP第一隐藏层维度：大幅增加以提高模型容量
MLP_HIDDEN_DIM_2 = 256         # MLP第二隐藏层维度：增加以匹配第一层
...
# 早停机制参数：防止过拟合，节省训练时间
EARLY_STOPPING_PATIENCE = 8     # 早停耐心值：连续30个epoch没有改善就停止训练（增加耐心）
MIN_DELTA = 1e-6                 # 最小改善阈值：改善必须大于0.0001才算有效（放宽阈值）
...
def optimize_gpu_performance():
    ...
    if torch.cuda.is_available():
        # 注意：不设置 torch.backends.cudnn.benchmark = True
        # 因为这会与 set_random_seeds() 中的确定性设置冲突
        # 为了保证结果可重现，我们保持 benchmark = False

        # 启用TensorFloat-32 (TF32) 以加速训练
...
def save_test_predictions(all_preds, all_targets, coin_names, model_name, test_metrics=None):
    ...
    功能：
        将模型在测试集上的预测结果保存为文件：
        1. 详细预测结果：每个样本每个币种的预测值、真实值和误差 (CSV)
        2. 统计信息：每个币种的统计指标汇总 (CSV)
        3. 格式化测试结果：完整的测试输出 (TXT)
    ...
    输出文件（保存在 experiments/cache/test_predictions/{model_name}/ 目录下）：
        - test_predictions.csv: 详细预测结果
        - test_statistics.csv: 统计信息汇总
        - test_results.txt: 格式化测试结果
    """
    # === 创建按模型名称分类的保存目录 ===
    base_save_dir = "experiments/cache/test_predictions"
    model_save_dir = os.path.join(base_save_dir, model_name)
    ...
    if test_metrics:
        ...
        print(f"  格式化结果: {results_txt_file}")
        ...
        for batch_data in tqdm(data_loader, desc="Evaluating"):
            ...
            if task_type == 'classification':
                ...
                if PREDICTION_TARGET == 'price':
                    # 直接预测价格：判断价格是否大于某个阈值
                    targets = (target_data > 0).long()  # 涨=1, 跌=0，形状: [batch_size, num_nodes]
                elif PREDICTION_TARGET in ('diff', 'return'):
                    # 预测价差/变化率：数据集中 target 已是“下一步的差值/收益率”；直接判断正负
                    targets = (target_data > 0).long()  # 上涨=1, 下跌=0，形状: [batch_size, num_nodes]
                ...
                if PREDICTION_TARGET == 'price':
                    ...
                    # 提取第一个预测步进行比较，确保维度匹配
                    targets = target_data
                    # 安全地处理输出维度：如果最后一维是1则squeeze，否则取第一个预测步
                    if outputs.shape[-1] == 1:
                        outputs = outputs.squeeze(-1)     # [B, C, 1] -> [B, C]
                    else:
    ...
    if task_type == 'classification':
        ...
        if PREDICTION_TARGET == 'price':
            if scaler:
                ...
                print(f"🔄 反归一化完成: 预测值形状={original_preds.shape}, 目标值形状={original_targets.shape}")
            else:
...
if __name__ == '__main__':
    ...
    print(f"⚠️  重要：为避免数据泄露，将在数据集划分后再进行标准化")
    ...
    class WPMixerConfigs:
        def __init__(self):
            ...
            self.pred_length = 1 # We predict a sequence
            ...
            self.use_amp = False
            self.device = DEVICE # 修复：添加device属性

    wpmixer_configs = WPMixerConfigs()

    print("🚀 初始化新的混合模型 ---")
    model = HybridReconstructionWPMixer(
        configs=wpmixer_configs,
        use_gcn=USE_GCN,
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        patience=10,        # 等待10个epoch没有改善再降低学习率（更宽容）
        factor=0.8,         # 学习率衰减因子：每次减少20%（更温和的衰减）
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch_data in enumerate(train_pbar):
            ...
            # === 模型前向传播：多模态输入融合 ===
            # 将价格序列、图结构、新闻特征输入WPMixer模型
            outputs = model(
                price_data=price_seq,        # 价格序列：主要的时序特征
                ...
                if PREDICTION_TARGET in ('diff', 'return'):
                    ...
                    if batch_idx == 0:
                        ...
                        with torch.no_grad():
                            ...
                            pred_stats = {
                                'pred_class_0': (pred_classes == 0).sum().item(),
                                'pred_class_1': (pred_classes == 1).sum().item(),
                                'avg_prob_class_0': pred_probs[:, :, 0].mean().item(),
                                'avg_prob_class_1': pred_probs[:, :, 1].mean().item(),
                                'max_prob_class_0': pred_probs[:, :, 0].max().item(),
                                'max_prob_class_1': pred_probs[:, :, 1].max().item()
                            }
                        ...
                        print(f"🔍 模型预测分布: 下跌={pred_stats['pred_class_0']}/{target_stats['total_samples']} ({pred_stats['pred_class_0']/target_stats['total_samples']:.1%}), 上涨={pred_stats['pred_class_1']}/{target_stats['total_samples']} ({pred_stats['pred_class_1']/target_stats['total_samples']:.1%})")
                        print(f"🔍 预测概率统计: 下跌平均={pred_stats['avg_prob_class_0']:.3f}(最大={pred_stats['max_prob_class_0']:.3f}), 上涨平均={pred_stats['avg_prob_class_1']:.3f}(最大={pred_stats['max_prob_class_1']:.3f})")

                        # 检查是否出现预测偏向问题
                        pred_ratio_0 = pred_stats['pred_class_0'] / target_stats['total_samples']
                        pred_ratio_1 = pred_stats['pred_class_1'] / target_stats['total_samples']
                        if pred_ratio_0 > 0.9 or pred_ratio_1 > 0.9:
                            print(f"⚠️ 警告：模型预测严重偏向一个类别！")
                            if USE_GCN:
                                print(f"� 建议：尝试降低GCN的影响或调整学习率")
                            else:
                                print(f"💡 建议：检查数据平衡性或调整损失函数")
                else:
                    raise ValueError(f"PREDICTION_TARGET '{PREDICTION_TARGET}' not supported for classification")
